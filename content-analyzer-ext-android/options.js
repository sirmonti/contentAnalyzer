...
function renderTable() {
    ...
    servicesList.forEach((srv, index) => {
        ...
        tr.innerHTML = `
            <td data-label="${api.i18n.getMessage("thName") || "Nombre"}">${srv.name}</td>
            <td data-label="${api.i18n.getMessage("thType") || "Tipo"}">${srv.type}</td>
            <td data-label="${api.i18n.getMessage("thDomains") || "Dominios vinculados"}">${domainsText}</td>
            <td data-label="${api.i18n.getMessage("thModel") || "Modelo"}">${srv.model}</td>
            <td>
...
function renderPromptsTable() {
    ...
    promptsList.forEach((p, index) => {
        ...
        const tr = document.createElement("tr");
        tr.innerHTML = `
            <td data-label="${api.i18n.getMessage("thName") || "Nombre"}">${p.name}</td>
            <td data-label="${api.i18n.getMessage("thPrompt") || "Prompt"}" style="color:var(--color-text-muted); font-size:0.9rem;">${previewText}</td>
            <td>
