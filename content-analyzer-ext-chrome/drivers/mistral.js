/**
 * @file drivers/mistral.js
 * @description Communication driver with the Mistral AI API.
 *
 * Mistral AI provides open and commercial LLMs (Mistral Large, Mistral Small, Codestral,
 * Pixtral, Ministral, etc.) via their official API endpoint (api.mistral.ai/v1).
 *
 * AUTHENTICATION:
 * --------------
 ...
 *     Lists available models in standard format.
 *
 ...
 * Supports `<think>...</think>` filtering for reasoning models.
 */
 ...
 * @param {string} config.apikey - Mistral API Key (Bearer token).
...
export async function fetchModels(config) {
    const fetchUrl = "https://api.mistral.ai/v1/models";
 ...
 * into a valid OpenAI/Mistral chat completions `messages` array with string content.
 *
...
/**
 * Generates responses from Mistral AI in streaming mode (SSE).
 *
 * @param {Object} config  - Service configuration.
 * @param {string} config.model  - Model ID (e.g.: "mistral-small-latest").
...
export async function* generate(config, prompt) {
    const fetchUrl = "https://api.mistral.ai/v1/chat/completions";
