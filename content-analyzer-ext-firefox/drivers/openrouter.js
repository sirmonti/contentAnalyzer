/**
 * @file drivers/openrouter.js
 * @description Communication driver with the OpenRouter API.
 *
 * OpenRouter provides unified access to a vast catalog of models (OpenAI, Anthropic,
 * Google, Meta, Mistral, DeepSeek, Cohere, etc.) via an OpenAI-compatible endpoint.
 *
 * AUTHENTICATION:
 * --------------
 ...
 * - `X-Title`: Application title for attribution
 *
 * ENDPOINTS USED:
 * -----------------
 * - GET  https://openrouter.ai/api/v1/models
 *     Lists all available models with their IDs (e.g., "anthropic/claude-3.5-sonnet").
 *
 * - POST https://openrouter.ai/api/v1/chat/completions
 *     Generates streaming completions with `stream: true`.
 *
 ...
 * @param {string} config.apikey - OpenRouter API Key (Bearer token).
 * @returns {Promise<Array<{name: string}>>} List of models with a `name` property (= model ID).
 * @throws {Error} If the HTTP response is an error.
 */
export async function fetchModels(config) {
    const fetchUrl = "https://openrouter.ai/api/v1/models";
 ...
 * into a valid OpenRouter chat completions `messages` array with string content.
 *
...
/**
 * Generates responses from OpenRouter in streaming mode (SSE).
 *
 * @param {Object} config  - Service configuration.
 * @param {string} config.model  - Model ID (e.g.: "anthropic/claude-3.5-sonnet").
...
export async function* generate(config, prompt) {
    ...
    const headers = {
        ...
        "HTTP-Referer": "https://github.com/montidev/contentAnalyzer",
                ...
                if (tLine.startsWith("data: ")) {
                    ...
                    try {
                        ...
                        if (json.choices && json.choices[0] && json.choices[0].delta) {
                            ...
                            const chunk = delta.content || delta.reasoning || delta.reasoning_content || "";
