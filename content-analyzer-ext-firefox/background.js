...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "fetchModels") {
    ...
    if (!driver) {
      sendResponse({ success: false, error: browser.i18n.getMessage("errUnsupportedService").replace("$1", "") });
      return false;
    }
    ...
    port.onMessage.addListener(async (msg) => {
      if (msg.action === "generate") {
        try {
          ...
          const driver = DRIVERS[type];
          if (!driver) throw new Error(browser.i18n.getMessage("errUnsupportedService").replace("$1", type));
